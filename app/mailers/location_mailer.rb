class LocationMailer < ApplicationMailer
  ...
  delegate :record_errors_for, to: LocationMailer

  def location_results(request, result_url:)
    raise ArgumentError unless request.output_file_uploaded?

    headers(to: request.email, subject: 'Your location request')
  ...
  def request_failed(request, errors:)
    headers(to: request.email, subject: 'Error processing location request')
