...
module Location
  describe BatchJob, type: :job do
    job_classes = [Location::BatchJob, Location::WorldCatJob, Location::HathiTrustJob, Location::ResultsJob, Location::RequestFailedJob]
    ...
    before do
      ...
      @result_url = url_helpers.location_requests_result_url(@req, host: 'framework.example.edu')
    end
    ...
    context 'failure' do
      ...
      context 'failure in WorldCat location retrieval' do
        before do
      ...
      context 'failure in results generation' do
        before do
          allow_any_instance_of(LocationRequest).to receive(:ensure_output_file!).and_raise(error)
        end
