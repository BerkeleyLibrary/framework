...
describe :galc_request_form, type: :request do
  it 'redirects to login' do
    expected_location = login_path(url: new_galc_request_form_path)
  ...
  describe 'with a valid user' do
    ...
    it 'rejects a submission with missing fields' do
      post('/forms/galc-agreement', params: {
             galc_request_form: {}
           })
      expect(response).to redirect_to(new_galc_request_form_path)
    end
    ...
    it 'accepts a submission' do
      ...
      expect(response).to redirect_to('/forms/galc-agreement/confirmed')
    end
    ...
    it 'redirects from :show to :new' do
      ...
      get galc_request_form_path(some_meaningless_id)
      expect(response).to redirect_to(new_galc_request_form_path)
    end
  end
end
