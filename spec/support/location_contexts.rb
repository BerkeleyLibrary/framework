...
RSpec.shared_context('location data') do
...
RSpec.shared_context('purge LocationRequests') do
  before do
...
RSpec.shared_context('LocationRequest') do
...
RSpec.shared_context('complete LocationRequest') do
  include_context('LocationRequest')

  before do
    ht_retrieved = true
    wc_retrieved = true
    req.location_records.find_each do |rec|
      oclc_number = rec.oclc_number
      wc_symbols = locations_by_oclc_num[oclc_number].join(',')
...
RSpec.shared_context('incomplete LocationRequest') do
  include_context('LocationRequest')

  before do
    req.location_records.find_each.with_index do |rec, i|
      oclc_number = rec.oclc_number

      options = {}.tap do |opts|
        if i.even?
          opts[:wc_retrieved] = true
          opts[:wc_symbols] = locations_by_oclc_num[oclc_number].join(',')
        end
...
RSpec.shared_context('incomplete LocationRequest with errors') do
  include_context('incomplete LocationRequest')

  before do
    req.location_records.where(wc_retrieved: true).find_each.with_index do |rec, i|
      rec.update(wc_symbols: nil, wc_error: '403 Forbidden') if i.even?
    end

    req.location_records.where(ht_retrieved: true).find_each.with_index do |rec, i|
      rec.update(ht_record_url: nil, ht_error: '500 Internal Server Error') if i % 3 == 0
    end
  end
end

RSpec.shared_context('complete LocationRequest with errors') do
  include_context('complete LocationRequest')

  before do
    req.location_records.find_each.with_index do |r, i|
...
RSpec.shared_context('LocationRequest with broken input file attachment') do
  include_context('LocationRequest')
...
RSpec.shared_context('LocationRequest with broken output file attachment') do
  include_context('complete LocationRequest')
