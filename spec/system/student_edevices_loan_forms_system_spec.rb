...
describe :student_edevices_loan_forms, type: :system do
  describe 'with a valid user' do
    ...
    it 'displays the patron email as a read-only field' do
      ...
      patron_email_field = find(:xpath, "//input[@id='student_edevices_loan_form_patron_email']")
    ...
    it "displays the user's display name as a read-only field" do
      ...
      display_name_field = find(:xpath, "//input[@id='student_edevices_loan_form_display_name']")
    ...
    it 'marks all checks as required' do
      checks = StudentEdevicesLoanForm.instance_methods.map(&:to_s).select { |n| n.end_with?('check') }
      checks.each do |check|
        check_field = find(:xpath, "//input[@id='student_edevices_loan_form_#{check}']")
    ...
    it 'accepts a submission' do
      ...
      expect(page.current_path).to eq('/forms/student_edevices_loan/all_checked')
    end
  end
end
